...
const ManageCollateral = () => {
      ...
      toast({
        ...
        description: (
          <div className="flex items-center gap-2">
            ...
            <span>Silakan pilih jenis crypto dan masukkan jumlah yang valid.</span>
          </div>
        ),
  ...
  const handleWithdrawal = () => {
    ...
    if (!selectedAsset || isNaN(amountToWithdraw) || amountToWithdraw <= 0 || amountToWithdraw > selectedAsset.amount) {
      toast({
        variant: "destructive",
        title: "Penarikan Gagal",
        description: "Jumlah penarikan tidak valid atau melebihi jaminan yang tersedia.",
        className: "border-red-500 bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-400",
      });
      return;
    }
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              <Tabs defaultValue="deposit" className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                  <TabsTrigger value="deposit"><ArrowUpCircle className="w-4 h-4 mr-2" /> Deposit</TabsTrigger>
                  <TabsTrigger value="withdraw"><ArrowDownCircle className="w-4 h-4 mr-2" /> Tarik</TabsTrigger>
                </TabsList>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-6">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input id="amount" placeholder="0,001" value={depositAmount} onChange={(e) => setDepositAmount(formatNumber(e.target.value))} />
                      </div>
                      <div className="flex items-center space-x-2 p-3 bg-gray-50 rounded-lg">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center">
                            ...
                            <Button size="sm" variant="outline" onClick={copyAddress} className="ml-2 h-8"><Copy className="w-4 h-4" /></Button>
                          </div>
                        </div>
                      </div>
                    ...
                    <CardContent className="space-y-6">
                          ...
                          <SelectContent>
                            {availableCollateral.map(asset => (
                              <SelectItem key={asset.type} value={asset.type}>
                                {asset.name} - Tersedia: {asset.amount.toLocaleString('id-ID', { maximumFractionDigits: 8 })} {asset.symbol.toUpperCase()}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="relative">
                          ...
                          />
                          <Button type="button" variant="ghost" size="sm" className="absolute right-1 top-1/2 -translate-y-1/2 h-8 px-3" onClick={handleSetMaxWithdrawal} disabled={!withdrawalCollateralType}>
                            Max
                          </Button>
                        </div>
                        {withdrawalError && (
                      ...
                      <Button className="w-full text-white" onClick={handleWithdrawal} disabled={!withdrawalAmount || !withdrawalCollateralType || loading || !!withdrawalError}>
              ...
              <Card className="flex flex-col">
                <CardHeader>
                  <CardTitle className="flex items-center"><Wallet className="w-5 h-5 mr-2" />Ringkasan Jaminan</CardTitle>
                      ...
                      <div key={asset.type} className="flex items-center justify-between p-3 border rounded-lg">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground">{asset.amount.toLocaleString('id-ID', { maximumFractionDigits: 8 })} {asset.symbol}</p>
                          </div>
                        </div>
            ...
            <Card className="mt-8">
              ...
              <CardContent>
                <Table>
                  <TableHeader>
                    <TableRow><TableHead>Tanggal</TableHead><TableHead>Tipe</TableHead><TableHead>Jumlah</TableHead><TableHead>Status</TableHead></TableRow>
                  </TableHeader>
                  <TableBody>
                    {transactionHistory.map((tx) => (
                      <TableRow key={tx.id}>
                        <TableCell>{tx.date}</TableCell>
                        <TableCell><Badge variant={tx.type === "Deposit" ? "default" : "secondary"} className={tx.type === "Deposit" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}>{tx.type}</Badge></TableCell>
      ...
      <Dialog open={isDepositConfirmOpen} onOpenChange={setDepositConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Anda akan melakukan deposit jaminan sebesar <span className="font-bold">{depositAmount} {depositCollateralType.toUpperCase()}</span>. Tindakan ini tidak dapat dibatalkan setelah diproses.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="mt-4">
            <DialogClose asChild><Button variant="outline">Batal</Button></DialogClose>
            {/* --- PERUBAHAN: Tombol konfirmasi dengan warna primer --- */}
            <Button className="bg-primary text-white hover:bg-primary/90" onClick={confirmDeposit}>Ya, Lanjutkan Deposit</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* --- PERUBAHAN: Dialog Konfirmasi Penarikan dengan struktur baru --- */}
      <Dialog open={isWithdrawConfirmOpen} onOpenChange={setWithdrawConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="mb-2">Konfirmasi Penarikan</DialogTitle>
            <DialogDescription>
              Anda akan menarik jaminan sebesar <span className="font-bold">{withdrawalAmount} {withdrawalCollateralType.toUpperCase()}</span>. Pastikan Anda telah memahami risikonya.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="mt-4">
            <DialogClose asChild><Button variant="outline">Batal</Button></DialogClose>
            {/* --- PERUBAHAN: Tombol konfirmasi dengan warna primer --- */}
            <Button className="bg-primary text-white hover:bg-primary/90" onClick={confirmWithdrawal}>Ya, Lanjutkan Penarikan</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
